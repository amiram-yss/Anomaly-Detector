...
void SimpleAnomalyDetector::learnNormal(const TimeSeries &ts) {
    ...
    for (int i = 0; i < columns; ++i) {
        for (int j = i + 1; j < columns; ++j) {
            ...
            p = fabsf(pearson(ts.getFeatureArray(i), ts.getFeatureArray(j), rows));
            if (p > m) {
                m = p;
                c = j;
            }

            //If there is a correlation and it is also greater than the threshold
            if (c != -1 && m >= BRINK) {
                this->cf.push_back(collideTwoFeatures(ts, i, c));
            }
        }
    }
}
